...
function Admin() {
    ...
    const { getUserProfile,profileData } = GetUsertPosts();
    ...
    const handlecloseStrorage = () => {
        // borrar todo de storage
        localStorage.clear();
...
<div className="text-gray-300 p-4 rounded-lg text-sm w-[400px] 
absolute inset-0 m-auto h-fit bg-[#161b22] border border-[#3f3f46]">
     <button className="bg-white mx-auto text-black p-2 block text-center w-full rounded-full" onClick={()=>handlecloseStrorage()}>
        cerrar sesion
    </button>
      ...
      />
		<div className="w-full text-gray-500">
			<p className="w-[200px] text-lg space-x-1 inline-block overflow-hidden whitespace-nowrap text-ellipsis">
				<span className="text-gray-100">Identificate</span><span className="font-extralight"></span>
			</p>
			<p>Cuenta gratis</p>
		</div>
	</div>
	...
	<div className="w-full flex flex-row space-x-4 p-4 text-gray-500 text-sm">
		<p className="flex items-center space-x-1">
			<a className="hover:text-blue-500 flex items-center space-x-1"
				href="#">Saldo: <span>COP 1000.0000</span><span> </span></a>
		</p>
	</div>
	<div className="w-full border-b border-[#3f3f46]"></div>
...
<div className="m-4">
    <h1 className="font-bold text-blue-300">Datos personales</h1>
{
    profileData && profileData.map((item) => {
        return (
            <div key={item.id} className="flex flex-col">
                <p>Nombre: {item.name ? item.name : "No hay nombre"}</p>
                <p>Apellido: {item.surname ? item.surname : "No hay apellido"}</p>
